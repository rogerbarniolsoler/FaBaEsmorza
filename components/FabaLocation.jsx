...
export default function FabaLocation({ esmorzar }) {
  ...
  return (
    <Marker
      ...
      // El tracksViewChanges a false millora moltíssim el rendiment del mapa quan hi ha molts marcadors
      tracksViewChanges={Platform.OS === 'ios' ? true : false} 
    >
      {/* Icona del Marcador (La teva fava) */}
      <View style={styles.markerIcon}>
        <Svg width="35" height="37" viewBox="0 0 116 123" fill="none">
      ...
      <Callout tooltip>
        <View style={styles.calloutContainer}>
          <View style={styles.calloutBurbuja}>
            {/* Títol del local */}
            <Text style={styles.titol} numberOfLines={1}>{esmorzar.nom}</Text>
...
const styles = StyleSheet.create({
  markerIcon: {
    ...
    // Podem afegir una miqueta d'ombra perquè la fava floti millor sobre el mapa
    shadowColor: '#000',
